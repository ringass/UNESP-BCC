...
// Funcao do exercicio-------------------------------------------------------------------------------------------------------------------
void AtualizarSal(TipoCadastro *p)
{
  char Rgcompare[10];
  int newSal;
  fgets(Rgcompare, sizeof(Rgcompare), stdin);

  for (int i = 0; i < p->qt; i++)
  {
    if (strcmp(Rgcompare, p->funcionarios[i].rg) == 0)
    {
      scanf("%f", &newSal);
      p->funcionarios[i].salario = newSal;
    }
  }
}
//------------------------------------------------------------------------------------------------------------------
...
int main()
{
  ...
  printf("\n\nFuncao atualizar salario de funcionario:");
  AtualizarSal(&empresa1);
