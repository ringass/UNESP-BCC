...
AnalisadorLexico::AnalisadorLexico(const std::string& nomeIn,
                                   const std::string& nomeOut,
                                   bool gerarArquivo)
    : gerarArquivo(gerarArquivo), proximo(' '), simbolo(0), linha(1)
{
...
void AnalisadorLexico::ERRO(const std::string& msg) {
    std::cerr << "[ERRO LEXICO] Linha " << linha << ": " << msg
              << " (caractere: '" << proximo << "')\n";
    exit(1);
}
...
Token AnalisadorLexico::proximoToken() {
    ANALISADOR_LEXICO();
    return Token(simbolo, atomo, linha);
}
...
void AnalisadorLexico::analisar() {
    ...
    std::cout << "\n[OK] Analise lexica concluida com sucesso!! "
              << linha << " linha(s) processada(s).\nArquivo "
              << nomeOut << " gerado.\n";
}
...
void AnalisadorLexico::imprimirArquivo(const Token& t) {
    ...
    else if (t.codigo >= 10 && t.codigo <= 31) {
