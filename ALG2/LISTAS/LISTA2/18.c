...
// Funcao do exercicio-------------------------------------------------------------------------------------------------------------------
void RemoverFuncionario(TipoCadastro *p)
{
int flag = -1;
char rg[10];
    scanf("%s", &rg);
    for (int i = 0; i < p->qt; i++) {
        if (strcmp(p->funcionarios[i].rg, rg) == 0) {
            flag = i;
            break;
        }
    }
    ...
    if (flag != -1) {
        for (int i = flag; i < p->qt - 1; i++) {
            p->funcionarios[i] = p->funcionarios[i + 1];
        }
        p->qt--;
        printf("Funcionario com RG %s foi removido.\n", rg);
    } else {
        printf("Funcionario com RG %s nao foi encontrado.\n", rg);
    }
}
//------------------------------------------------------------------------------------------------------------------
